...
function excerpt_read_more_link($output) {
 global $post;
 return $output . '<a href="'. get_permalink($post->ID) . '" class="btn"> Read More...</a>';
}
...
function bootstrap_method() {
   // register your script location, dependencies and version
   wp_register_script('bootstrap',
       get_template_directory_uri() . '/js/bootstrapAll.js',
       array('jquery'),
       '1.0' );
   // enqueue the script
   wp_enqueue_script('bootstrap');
}
add_action('wp_enqueue_scripts', 'bootstrap_method');
